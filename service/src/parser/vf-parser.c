...
static vf_pixel_format_t vf_parser_str_to_format(const char *str)
{
...
static char *vf_parser_read_file(const char *path)
{
    FILE *f = NULL;
    char *buffer = NULL;
    size_t read_size = 0U;
...
static int vf_parser_extract_string(char *src, char *out, size_t max_len)
{
    char *colon = NULL;
    char *start = NULL;
    char *end = NULL;
...
static int vf_parser_extract_int(char *src, int *value)
{
    char *colon = NULL;
